...
self.slozka =os.getcwd()+'/ORTEC'
self.ampl1=-1000
self.spektrum={}

self.vyhodnocovat_gama=0 #Vyžadováno připojení k internetu
self.pocet_jader=cpu_count()

def Multi(self,i1):
    
    # global self.spektrum
    
    if system()=='Windows':
        print(u'%s' %os.path.basename(self.soubor[i1]))
    else:
        print(u'%s' %self.soubor[i1].replace(self.path0 + '/', ''))
...
def Forty_Three(self):
    ...
    self.vyhlazeni=0 #zda vyhlazovat samotné spektrum
    ...
    grafy,self.typ_pozadi,self.sirka,self.cykl,config0,config1 = 0, 2, 1, 8, ['1085.7', '510.9989'], ['2783.6', '1460.83'] 
    if self.slozka:
    ...
    # print(u'Zpracovávám následující soubory:')
    ...
    if self.vyhodnocovat_gama==1:
        ...
        for i1 in range(0,len(self.soubor)):
            ...
            if prvek1 not in self.Gama_data.keys(): # Data o gama linkách z internetu jsou ukládány do dictionary self.Gama_data a není tedy potřeba je stahovat pro každý isotop zvlášť.
                GAMA=get_gamma_data.Data.get_gamma(self,prvek1)
                self.Gama_data[prvek1]=GAMA
            
    
    
    # print(len(self.soubor),cpu_count())
    Parallel(n_jobs=self.pocet_jader)(delayed(Multi)(self,i1) for i1 in range(0,len(self.soubor)))
    
        
    ## Vykreslování grafů spekter a pozadí 
    
    if (grafy==2):
        print(u'Hotovo! Zobrazuji grafy.')
